...
- 关键指标：周复盘完成率、AI WAM 审批完成率、W7–W12 锁定合规率、**85% 目标达成率**（85-90% 区间覆盖率）、R7/R30 留存。
- 激活：首 48 小时完成愿景草案 ≥ 70%。
- 执行：周复盘完成率 ≥ 60%；平均周得分达到 85% 或落在 85-90% 区间的覆盖率 ≥ 50%。
- 监督：WAM 出席率 ≥ 70%。
...
- 完成率目标区间 **85%**（最佳学习区）可视化；<85% 建议减负拆小（目标太难），>90% 建议适度加码（目标太容易），85-90% 为理想学习区间。
...
# 6. 完整工作流程（基于 Zach 方法论）

## 6.1 Phase 1: Finding Your Vision（开始前 1 个月）

**目标**：明确理想的一天、一周、一年和人生愿景，为后续行动提供方向。

**流程**：
1. **Daily Vision（每日愿景）**：详细描述理想的一天（起床时间、地点、活动、感受等）
2. **Weekly Vision（每周愿景）**：扩展为理想的一周（工作天数、旅行频率、周常活动、家庭/朋友关系）
3. **Year/Life Vision（年度/人生愿景）**：基于每日和每周愿景，提炼长期目标（家庭、事业、财务、生活方式）

**产品实现**：
- `/vision` 页面提供引导问题，分四级填写
- 支持标签（tags）标记核心价值观/关注领域
- 愿景与计划任务对齐检测（在 Plan/This Week 页面显示对齐提示）

**时间投入**：新用户 30–40 分钟完成草案

---

## 6.2 Phase 2: Execute（第 1–3 个月）

### 6.2.1 创建 12 周计划（开始前 1 周）

**目标**：将愿景转化为可执行的 12 周行动计划。

**流程**：
1. 设定 **1 Personal + 1 Professional** 目标（仅允许各 1 个）
2. 将目标拆解为**输入型行动**（完全可控，如"写 6 个视频脚本"而非"获得 500 粉丝"）
3. 为每周（1–12 周）规划具体任务（bullet points，按个人/职业分组）
4. 设置锁定规则：**W1–W6 可调整，W7–W12 锁定**（首次执行时前 6 周可调）

**产品实现**：
- `/plan` 页面：12 周网格视图，支持 AI 生成、手动编辑、一键导入
- 输入约束检测：阻止 KPI/输出型目标，引导改写为输入型
- 锁定保护：W7–W12 自动置灰，后端强校验

**时间投入**：24 小时内产出完整计划

---

### 6.2.2 每周执行循环（Week 1–12）

**每周流程**（以周日为规划日为例）：

**周日（Review & Planning Day）**：
1. **回顾上周**：
   - 计算完成度百分比（完成项/计划项 × 100%）
   - 评估是否达到 **85% 目标**（85–90% 为理想区间）
   - 识别障碍（时间/精力/环境/技能）
   - 记录胜利清单和失败模式
2. **AI WAM（Weekly Accountability Meeting）**：
   - 进入 `/wam` 页面，启动 AI 周会
   - AI 热身（情绪/能量条）→ 用户分享 → AI 呈现事实卡 → 共同诊断 → 生成提案卡
   - 用户审批提案（Approve/Edit/Reject）→ 执行变更 → 导出纪要
   - **时间投入**：30–45 分钟
3. **规划下周**：
   - 选择工作流模式（Time Blocking 或 Priority Queue）
   - 如使用 Time Blocking：为任务分配具体时间块
   - 如使用 Priority Queue：按优先级排序任务

**周一至周六（执行日）**：
- 在 `/dashboard`（This Week）页面跟踪执行
- 更新任务状态（Planned → In Progress → Completed → Skipped）
- 记录完成数量
- 随时记录障碍（Quick Notes）

**关键原则**：
- **85% 规则**：目标完成度应接近 85%（低于 85% 说明目标太难，需减少；高于 90% 说明目标太容易，需增加）
- **信任过去的自己**：W7–W12 锁定后，不再修改配额，专注执行
- **输入优先**：只关注可控的行动，不关注结果/KPI
...
**首次执行（First Time）**：
- **W1–W6**：可调整任务和配额（学习期，允许校准）
- **W7–W12**：锁定，不可修改配额（信任过去的自己）

**后续周期（After First Cycle）**：
- 基于上一周期的完成度分布，调整下一周期的任务量
- 完成度 >90%：下一周期可增加任务
- 完成度 <80%：下一周期需减少任务
- 完成度 85–90%：保持当前节奏
...
## 6.3 Phase 3: Reevaluate and Think Big（第 4–6 个月）

### 6.3.1 12 周复盘（Week 12 结束后）

**目标**：全面回顾过去 12 周的执行情况，为下一轮做准备。

**流程**：
1. **数据回顾**：
   - 12 周完成度分布图
   - 85% 目标达成率统计
   - 失败模式分析（时间/精力/环境/技能障碍）
   - 胜利清单汇总
2. **输入 vs 输出反思**：
   - 回顾输入型行动的执行情况
   - 观察输出结果（仅作回顾，不作为下一轮配额）
   - 识别输入与输出的关联性
3. **生成下一轮草案**：
   - 复制当前周期结构
   - 根据完成度分布调整任务量
   - 保持 1+1 目标和输入型约束
4. **导出复盘纪要**：Markdown/CSV 格式，便于外部复盘

**产品实现**：
- `/retro` 页面：图表 + 问答引导 + 一键生成下一轮
- 默认推荐 **2 周休整模式**（轻量跟踪，无配额）

**时间投入**：3–4 小时（建议与朋友一起，在自然环境中进行）
...
**流程**：
- 启用"休整模式"（轻量跟踪，无配额）
- 建议活动：阅读、散步、旅行、与家人朋友相处
- 避免制定新的行动计划

**产品实现**：
- 休整模式视图：仅显示轻量建议，无完成度追踪

---

### 6.3.3 进入下一轮（Month 4–6）

**流程**：
1. 完成休整后，回到 `/plan` 页面
2. 基于复盘结果，创建新的 12 周计划
3. 重复 Phase 2 的执行循环
...
开始前 1 周：Plan（12 周计划）
    ↓
Week 1–12：每周循环
    周日：Review + AI WAM + Planning
    周一至周六：Execute（This Week）
    ↓
Week 12 结束后：Retro（复盘）
    ↓
休整 2 周
    ↓
进入下一轮（Month 4–6）
...
- **侧边栏导航**：**Vision · Plan (12-week) · This Week · AI WAM · Retro · Settings**
...
- **结果指标**：周完成度达到 **85%** 或落在 **85–90% 区间**的比例、连续 4 周留存、进入下一轮比例。
